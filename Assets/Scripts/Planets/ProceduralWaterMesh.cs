...
namespace Sporidia.Planets
{
	...
	public class ProceduralWaterMesh : MonoBehaviour
	{
		...
		private Transform[] _WaveSources;
		private Mesh _Mesh;
		private float _LocalTime;

		// Buffers for CPU code path
		private NativeArray<Vector3> _WaveSourcePositions;
		private NativeArray<Vector3> _Vertices;

		// Buffers for GPU compute shader path
		private GraphicsBuffer _GpuWaveSourcePositions;
		private GraphicsBuffer _GpuVertices;

		private GUIContent[] _UIOptions;

		private void OnEnable()
		{
			_Mesh = CreateMesh();
			_WaveSources = transform.Cast<Transform>().Where(t => t.gameObject.activeInHierarchy).ToArray();
			_WaveSourcePositions = new NativeArray<Vector3>(_WaveSources.Length, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
		}

		private void OnDisable()
		{
			_WaveSourcePositions.Dispose();
			_Vertices.Dispose();
		...
		{
			_GpuWaveSourcePositions?.Dispose();
			_GpuWaveSourcePositions = null;
			_GpuVertices?.Dispose();
			_GpuVertices = null;
		}

		public void Update()
		{
			_LocalTime += Time.deltaTime * 2.0f;
		...
		private void UpdateWaveCpu()
		{
			...
			var job = new WaveJob { vertices = _Vertices, waveSourcePositions = _WaveSourcePositions, time = _LocalTime };
			...
			else if (mode == Mode.CPUBurst)
			{
				...
				job.Schedule(_Vertices.Length, _Vertices.Length).Complete();
			}
		...
		private void UpdateWaveGpu()
		{
			// Create GPU buffer for wave source positions, if needed.
			_GpuWaveSourcePositions ??= new GraphicsBuffer(GraphicsBuffer.Target.Structured, _WaveSources.Length, 12);
			...
			_GpuVertices ??= _Mesh.GetVertexBuffer(0);

			_GpuWaveSourcePositions.SetData(_WaveSourcePositions);

			waveComputeShader.SetFloat("gTime", _LocalTime);
			waveComputeShader.SetInt("gVertexCount", _Mesh.vertexCount);
			waveComputeShader.SetInt("gWaveSourceCount", _WaveSources.Length);
			waveComputeShader.SetInt("gVertexGridX", surfaceWidthPoints);
			waveComputeShader.SetInt("gVertexGridY", surfaceLengthPoints);

			// update vertex positions
			waveComputeShader.SetBuffer(0, "bufVertices", _GpuVertices);
			waveComputeShader.SetBuffer(0, "bufWaveSourcePositions", _GpuWaveSourcePositions);
			waveComputeShader.Dispatch(0, (_Mesh.vertexCount + 63) / 63, 1, 1);

			// calculate normals
			waveComputeShader.SetBuffer(1, "bufVertices", _GpuVertices);
			waveComputeShader.Dispatch(1, (_Mesh.vertexCount + 63) / 63, 1, 1);
		}

		private void UpdateWaveSourcePositions()
		{
			for (var i = 0; i < _WaveSources.Length; ++i)
				_WaveSourcePositions[i] = _WaveSources[i].position;
		}
		...
		private Mesh CreateMesh()
		{
			...
			_Vertices = new NativeArray<Vector3>(surfaceWidthPoints * surfaceLengthPoints, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
			var index = 0;
			for (var i = 0; i < surfaceWidthPoints; i++)
			{
				for (var j = 0; j < surfaceLengthPoints; j++)
				{
					...
					_Vertices[index++] = new Vector3(x, 0f, z);
				}
			}
		...
		public void OnGUI()
		{
			...
			mode = (Mode)GUILayout.Toolbar((int)mode, _UIOptions);
			GUILayout.Label($"Water: {surfaceWidthPoints}x{surfaceLengthPoints}, {_WaveSources.Length} wave sources");
			GUILayout.EndArea();
		}
	}
}