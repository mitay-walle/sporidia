...
namespace Hierarchy2
{
    ...
    public sealed class HierarchyEditor
    {
        ...
        private Texture _monoIcon;
        private Texture _attentionIcon;
        ...
        void EditorAwake()
        {
            _monoIcon = EditorGUIUtility.IconContent("cs Script Icon").image;
            _attentionIcon = EditorGUIUtility.IconContent("CollabError").image;
        ...
        void HierarchyOnGUI(int selectionID, Rect selectionRect)
        {
            ...
            else
            {
                ...
                var layer = rowItem.gameObject.layer;
                if (settings.displayLayer && layer != 0 && (_lastLayer != layer || rowItem.gameObject.transform.GetSiblingIndex() == 0))
                {
                    _lastLayer = layer; 
                    if (!settings.onlyDisplayWhileMouseEnter ||
                ...
                try
                {
                    ...
                    if (comType != null)
                    {
                        ...
                        if (isMono)
                        {
                            ...
                            {
                                var icon = EditorGUIUtility.ObjectContent(component, comType).image;
                                monoHasIcon.Add(comType,icon && icon != _monoIcon);
                            }

                            bool attentionIcon = ((MonoBehaviour)component) is IAttentionIconBehaviour;

                            bool shouldIgnoreThisMono = !monoHasIcon[comType] && !attentionIcon;
        ...
        void ComponentIcon(UnityEngine.Object component, Type componentType, Rect rect, bool isMaterial = false)
        {
            ...
            if (currentEvent.type == EventType.Repaint)
            {
                ...
                if (image?.name == "d_cs Script Icon")
                {
                    image = null;
                }
                
                if (component is IAttentionIconBehaviour beh && beh.DrawAttentionIcon)
                {
                    image = _attentionIcon;
                    tooltip = $"<b>{componentType.Name}</b> | {beh.Tooltip}";
                }
