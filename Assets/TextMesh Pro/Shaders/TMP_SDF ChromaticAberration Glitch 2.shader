Shader "TextMeshPro/Distance Field ChromaticAberration Glitch 2" {
...
Properties {
	...
	_UVNoiseMask        ("UV Noise Mask", 2D) = "black" {}
}
...
SubShader {
	...
	Pass {
		...
		fixed4 PixShader(pixel_t input) : SV_Target
		{
            ...
            {
                float2 uv = input.textures.xy;
            	uv.x += _Time.y * _GlitchMoveSpeed;
                float4 offset2 = tex2D(_UVNoiseMask, uv * float2(.1,.1));
                input.atlas += offset2.rg * offset2.a * .01;
            }
